...
template <class Key>
class BinaryTree {
protected: // implementation-of-dependency ===========================================
    class Node {
        public:
        Node() = default;
...
protected: // implementation-of-dependency ===========================================
    ...
    class TreeConstIterator : public BinaryTree<Key>::TreeIterator {
    public:
        ...
        TreeConstIterator& operator=(TreeConstIterator &iter) noexcept {
            this->current_node = iter.current_node;
            this->_begin = iter._begin;
            this->_end = iter._end;
            this->_size = iter._size;
            return *this;
        }
        TreeConstIterator& operator=(TreeConstIterator &&iter) noexcept {
            return *this = iter;
...
public: // BinaryTree-implementation =================================================
    BinaryTree() {
...
public:
    ...
    void erase(iterator pos) {
        if (!pos.current_node->_left && !pos.current_node->_right) {
            DeleteNodeWithoutChild(pos.current_node);
        } else if (!pos.current_node->_left || !pos.current_node->_right) {
            DeleteNodeWithOneChild(pos.current_node);
        } else {
            // дописать функцию по удалению с двумя детьми
        }
    }
    ...
    s21::pair<iterator, iterator> equal_range(const key_type& key) {
        return s21::pair<iterator, iterator>(lower_bound(key), upper_bound(key));
    }
